...
router.post("/quote", async (req, res) => {
  ...
  try {
    ...
    if (!["flights", "hotels", "sightseeing", "transfers"].includes(module)) {
      return res.status(400).json({
        error:
          "Invalid module. Must be: flights, hotels, sightseeing, transfers",
        code: "INVALID_MODULE",
      });
    }
    ...
    try {
      ...
      const fallbackResult = {
        ...
        finalPrice: shouldAccept
          ? userOffer
          : Math.max(minPrice, userOffer * (1.1 + Math.random() * 0.1)),
        basePrice: basePrice,
    ...
    setImmediate(() => {
      db.query(
        ...
        [
          ...
          result.basePrice
            ? (result.basePrice - userOffer) / result.basePrice
            : null,
...
router.post("/accept", async (req, res) => {
  try {
    ...
    if (
      session.result_price &&
      Math.abs(session.result_price - finalPrice) > 0.01
    ) {
      return res.status(409).json({
  ...
  try {
    ...
    if (status.hold_status === "booked") {
      ...
      if (status.seconds_left > 0) {
        ...
        await db.query("UPDATE bargain_holds SET status = $1 WHERE id = $2", [
          "expired",
          status.hold_id,
        ]);
      }
      ...
      new Date(status.session_expires) < new Date()
    ) {
      state = "expired";
    }
