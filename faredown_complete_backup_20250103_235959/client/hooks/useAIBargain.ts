...
export interface UseAIBargainReturn {
  ...
  setSuccessCallback: (
    callback: (finalPrice: number, orderRef: string) => void,
  ) => void;
...
export function useAIBargain(): UseAIBargainReturn {
  ...
  const successCallbackRef = useRef<
    ((finalPrice: number, orderRef: string) => void) | null
  >(null);
  const failureCallbackRef = useRef<(() => void) | null>(null);

  const startBargain = useCallback(
    (params: {
      module: "flights" | "hotels" | "sightseeing" | "transfers";
      title: string;
      productDetails: BargainProductDetails;
  ...
  const onBargainSuccess = useCallback(
    (finalPrice: number, orderRef: string) => {
...
export const createSightseeingBargainDetails = (
  ...
  basePrice:
    ticketType?.price ||
    attraction.price ||
    attraction.ticketPricing?.[0]?.price ||
    0,
