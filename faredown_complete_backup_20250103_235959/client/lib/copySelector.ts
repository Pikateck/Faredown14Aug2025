...
export function chooseVariant(
  ...
  const tone =
    ctx.tone ??
    (ctx.attempt === 1 ? "info" : ctx.attempt === 2 ? "firm" : "urgent");
  const bucket = (pack?.modules?.[ctx.module]?.[ctx.beat]?.[
    String(ctx.attempt)
  ] ||
