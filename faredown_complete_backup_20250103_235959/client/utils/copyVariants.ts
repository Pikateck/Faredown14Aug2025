...
class CopyVariantSelectorImpl implements CopyVariantSelector {
  ...
  chooseVariant(
    ...
    try {
      ...
      // For agent_offer, use attempt-specific variants
        ...
        variants =
          beatData[attemptKey] || beatData["1"] || beatData["any"] || [];
      }
      ...
      const totalWeight = poolToUse.reduce(
        (sum, variant) => sum + (variant.w || 1),
        0,
      );
  ...
  getFallback(
    ...
    try {
      ...
      const totalWeight = speakerFallbacks.reduce(
        (sum: number, variant: any) => sum + (variant.w || 1),
        0,
      );
      let random = Math.random() * totalWeight;
...
// Singleton instance
export const copyVariantSelector: CopyVariantSelector =
  new CopyVariantSelectorImpl();
...
export function getCopyVariantWithCurrency(
  ...
  if (formatPrice) {
    Object.keys(processedVariables).forEach((key) => {
      if (
        typeof processedVariables[key] === "number" &&
