...
const ConversationalBargainModal: React.FC<Props> = ({
  ...
  const startNegotiation = async () => {
    ...
    // Add user message
    addMessage("user", `I'd like to pay ₹${userPriceNum.toLocaleString()}`, userPriceNum);
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          {!showOfferActions && !isComplete && (
            <div className="p-4 bg-white border-t border-gray-200">
              <div className="mb-3">
                <div className="text-xs font-semibold text-gray-600 mb-2">Current price: ₹{(selectedFareType?.price || flight.price).toLocaleString()}</div>
                <div className="relative">
